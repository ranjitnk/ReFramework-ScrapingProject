...
**SAP element selection rule:** Before saving `$EREF`, check the candidate elements for the `[sap]` tag. If both a plain HTML element and a `[sap]` element match the target description, always pick the `[sap]` element — even if the plain element looks like a more direct match (e.g., a plain `INPUT` vs a SAP-tagged `InputBox` wrapping it).
...
The recapture overwrites `tree.md`. Re-grep against it and continue matching.
