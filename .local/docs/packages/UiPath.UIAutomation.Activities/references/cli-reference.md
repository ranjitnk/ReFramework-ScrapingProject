...
|--------|-------|-------------|----------|
| `folder-path` | `--folder-path <folder-path>` | The folder path for input/output data. | **Yes** |
| `refs` | `--refs <refs>` | Element reference(s), comma-separated. Each entry is `eREF:definition-file-path` (e.g., `e28:path1.json,e30:path2.json`). | **Yes** |
...
uip rpa uia selector-intelligence resolve-default-selector \
  --folder-path "path/to/data" \
  --refs "e42:path/to/target.json"

# Get selectors for multiple elements with per-ref definition file paths
uip rpa uia selector-intelligence resolve-default-selector \
  --folder-path "path/to/data" \
